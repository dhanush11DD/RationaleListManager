...
import { SpecialtyForm } from "./decisionForm";
...
export default function DecisionListPage({}: Props) {
  ...
  const clickDeleteBtn = async (row) =>{
    try{
      ...
      const result = await apiClient.delete(`/decision-list/${row.id}`)
  ...
  const columns: ColumnDef<DecisionList>[] = [...DecisionListColumns , actionBtn];
