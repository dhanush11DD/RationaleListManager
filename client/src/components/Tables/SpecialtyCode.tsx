...
import {SpecialtyCodeColumns} from "@/constants/TableColumns";
...
export default function SpecialtyCodePage({}: Props) {
  ...
  const clickDeleteBtn = async (row) =>{
    try{
      ...
      const result = await apiClient.delete(`/specialty-code/${row.id}`)
  ...
  const columns: ColumnDef<SpecialtyCode>[] = [...SpecialtyCodeColumns , actionBtn];
    ...
    <div className="flex flex-col gap-5  w-full">
        ...
        <div className="flex w-full justify-between">
          <PageTitle title="Specialty Code" />
            <Button onClick={()=>setSheetOpened(true)} className="">New Specialty Code</Button>
        </div>
